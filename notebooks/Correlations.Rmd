...
```{r relative_abundances, message=FALSE, warning=FALSE}
...
p.gene.rels <- ggplot(data=gene.rels.top10) + 
  ...
  facet_wrap(~fct_reorder(hostGroup2, hostOrder), scales='free_x', ncol=4) + 
  theme_light() + 
...
```{r plot_edge_distributions, message=FALSE, warning=FALSE}
all.p.edges <- list()
for (host in hosts) {
...
```{r avg_network_corrs, fig.height=15, fig.width=15, message=FALSE, warning=FALSE}
